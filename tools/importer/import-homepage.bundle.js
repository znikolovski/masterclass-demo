...
var CustomImportScript = (() => {
  ...
  function transform(hookName, element, payload) {
    ...
    if (hookName === TransformHook.afterTransform) {
      WebImporter.DOMUtils.remove(element, [".skip-link", ".navbar", "footer.footer"]);
    }
  }
  ...
  function transform2(hookName, element, payload) {
    if (hookName === TransformHook2.afterTransform) {
      ...
      const robustSelectors = {
        "section-1": "section.hero-section",
        "section-2": "section.secondary-section:has(.featured-article)",
        "section-3": "section:has(.tab-container)",
        "section-4": ".ticker-strip",
        "section-5": "section.inverse-section:has(.button-group):not(:has(.grid-images))",
        "section-6": "section:has(.faq-list)",
        "section-7": "section.secondary-section:has(.editorial-index)",
        "section-8": "section.inverse-section:has(.grid-images)",
        "section-9": "section.accent-section"
      };
      for (let i = sections.length - 1; i >= 0; i--) {
        const section = sections[i];
        const selector = robustSelectors[section.id] || section.selector;
