...
|---------|----------------|-----|----------|
| **Experience Workspace** Library → Blocks/Templates | `library/blocks.json`, `library/templates.json` | Content (`content.da.live`) | EW block picker — index must be on content bus so `da.live` can fetch without CORS |
...
| EW shows templates, not blocks | Index fetch fails (CORS if index is on `.aem.page`; use content bus) or rows missing `name`/`path` | DA config index → `content.da.live/.../library/blocks.json`; run `library:setup` |
